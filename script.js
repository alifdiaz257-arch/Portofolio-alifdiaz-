...
const stepsData = [
    ...
    { number: 5, icon: "cloud_upload", title: "Upload ke GitHub", desc: "Upload hasil scraping langsung ke repository GitHub Anda" },
...
function removeSkeletonClasses() {
    // Remove skeleton classes from all elements
    document.querySelectorAll('.skeleton-stat, .skeleton-btn, .skeleton-input, .skeleton-select, .skeleton-status, .skeleton-stars, .skeleton-rating, .skeleton-textarea').forEach(el => {
        el.classList.remove('skeleton-stat', 'skeleton-btn', 'skeleton-input', 'skeleton-select', 'skeleton-status', 'skeleton-stars', 'skeleton-rating', 'skeleton-textarea');
...
// Close welcome modal with close button
const welcomeCloseBtn = document.getElementById('welcomeCloseBtn');
if (welcomeCloseBtn) {
    welcomeCloseBtn.addEventListener('click', closeWelcomeModal);
}

// Start Scraper from Welcome Modal
...
// Click outside welcome modal to close
...
function verifyPin() {
    ...
    if (pinInput === ADMIN_PIN) {
        ...
        pinError.innerText = '❌ PIN salah! Silakan coba lagi.';
        pinError.style.animation = 'shake 0.3s ease';
        setTimeout(() => { pinError.style.animation = ''; }, 300);
    }
}

const pinAccessBtn = document.getElementById('pinAccessBtn');
if (pinAccessBtn) {
    pinAccessBtn.addEventListener('click', showPinModal);
}
...
if (closePinModalBtn) closePinModalBtn.addEventListener('click', hidePinModal);
...
function loadSteps() {
    ...
    setTimeout(() => {
        ...
        // Remove skeleton class from buttons
...
navItems.forEach(item => {
    item.addEventListener('click', (e) => {
        ...
        switch(pageId) {
            ...
            case 'github': title = 'Upload ke GitHub'; break;
        }
...
let themeStep = localStorage.getItem('themeStep') || '0';
...
if (currentTheme === 'dark') applyTheme('1');
else if (currentTheme === 'black-white') applyTheme('2');
else applyTheme('0');
...
if (scrapeBtn) {
    scrapeBtn.addEventListener('click', async () => {
        ...
        scrapingStatus.innerHTML = '<div class="loading-spinner"><span class="material-icons" style="animation: spin 1s linear infinite;">sync</span> Scraping in progress...</div>';
...
if (uploadBtn) {
    uploadBtn.addEventListener('click', async () => {
        ...
        uploadStatus.innerHTML = '<div class="loading-spinner"><span class="material-icons" style="animation: spin 1s linear infinite;">sync</span> Uploading to GitHub...</div>';
...
function updateDashboard() {
    ...
    const totalFilesElem = document.getElementById('totalFiles');
    const totalUploadsElem = document.getElementById('totalUploads');
    
    if (totalScrapesElem) totalScrapesElem.innerText = scrapeHistory.length;
    if (totalFilesElem) totalFilesElem.innerText = fileCount;
    if (totalUploadsElem) totalUploadsElem.innerText = localStorage.getItem('totalUploads') || '0';
...
function init() {
    ...
    // Remove skeleton classes after everything loads
    setTimeout(() => {
        removeSkeletonClasses();
    }, 1000);
